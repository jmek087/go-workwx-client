...
type ExternalContact struct {
	...
	Type ExternalUserType `json:"type,omitempty"`
...
type ExternalAttr struct {
	...
	Text ExternalAttrText `json:"text,omitempty"`
	// Web 网页类型的属性，url和title字段要么同时为空表示清除该属性，要么同时不为空 ，type为1时必填
	Web ExternalAttrWeb `json:"web,omitempty"`
	// Miniprogram 小程序类型的属性，appid和title字段要么同时为空表示清除改属性，要么同时不为空 ，type为2时必填
	Miniprogram ExternalAttrMiniprogram `json:"miniprogram,omitempty"`
}
...
type FollowUserInfo struct {
	...
	// AddWay 该成员添加此客户的来源
	AddWay FollowUserAddWay `json:"add_way,omitempty"`
...
type FollowUserTag struct {
	...
	Type FollowUserTagType `json:"type,omitempty"`
}
...
type ExternalContactCorpTagGroup struct {
	...
	Deleted bool `json:"deleted,omitempty"`
	// Tag 标签组内的标签列表
	Tag []ExternalContactCorpTag `json:"tag,omitempty"`
}
...
type ExternalContactUnassignedList struct {
	// Info 离职成员的客户
	Info []ExternalContactUnassigned `json:"info,omitempty"`
	// IsLast 是否是最后一条记录
...
type ExternalGroupChatJoinWay struct {
	...
	ChatIDs []string `json:"chat_id_list,omitempty"`
...
type Attachments struct {
	...
	MsgType AttachmentMsgType `json:"msgtype,omitempty"`
	// Image 图片消息配置
	Image Image `json:"image,omitempty"`
	// Link 图文消息配置
	Link Link `json:"link,omitempty"`
