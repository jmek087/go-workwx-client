...
type KfAccount struct {
	...
	ManagePrivilege bool `json:"manage_privilege,omitempty,omitempty"`
}
...
type KfServicer struct {
	...
	DepartmentID int64 `json:"department_id,omitempty,omitempty"`
}
...
type KfMsg struct {
	// MsgID 消息ID
	MsgID string `json:"msgid,omitempty,omitempty"`
	// OpenKfID 客服账号ID（msgtype为event，该字段不返回）
	OpenKfID string `json:"open_kfid,omitempty,omitempty"`
	// ExternalUserID 客客户UserID（msgtype为event，该字段不返回）
	ExternalUserID string `json:"external_userid,omitempty,omitempty"`
	// SendTime 消息发送时间
	SendTime int64 `json:"send_time,omitempty,omitempty"`
	// Origin 消息来源。3-微信客户发送的消息 4-系统推送的事件消息 5-接待人员在企业微信客户端发送的消息
	Origin int `json:"origin,omitempty,omitempty"`
	// ServicerUserID 从企业微信给客户发消息的接待人员userid（即仅origin为5才返回；msgtype为event，该字段不返回）
	ServicerUserID string `json:"servicer_userid,omitempty,omitempty"`
	// MsgType 消息类型
	MsgType MessageType `json:"msgtype,omitempty"`
	// Text 文本消息
	Text Text `json:"text,omitempty,omitempty"`
	// Image 图片消息
	Image Image `json:"image,omitempty,omitempty"`
	// Link 链接消息
	Link Link `json:"link,omitempty,omitempty"`
	// MiniProgram 小程序消息
	MiniProgram MiniProgram `json:"mini_program,omitempty,omitempty"`
	// Event 事件类型
	Event KfEvent `json:"event,omitempty,omitempty"`
}

// KfEvent 客服会话事件
type KfEvent struct {
	// EventType 事件类型
	EventType KfEventType `json:"event_type,omitempty"`
	// OpenKfID 客服账号ID
	OpenKfID string `json:"open_kfid,omitempty"`
	...
	Scene string `json:"scene,omitempty,omitempty"`
	// SceneParam 用户进入会话事件特有。进入会话的自定义参数，获取客服账号链接返回的url，开发者按规范拼接的scene_param参数
	SceneParam string `json:"scene_param,omitempty,omitempty"`
	// WelcomeCode 用户进入会话事件特有。如果满足发送欢迎语条件（条件为：用户在过去48小时里未收过欢迎语，且未向客服发过消息），会返回该字段。可用该welcome_code调用发送事件响应消息接口给客户发送欢迎语。
	WelcomeCode string `json:"welcome_code,omitempty,omitempty"`
	// WechatChannels 用户进入会话事件特有。进入会话的视频号信息，从视频号进入会话才有值
	WechatChannels KfWechatChannels `json:"wechat_channels,omitempty,omitempty"`
	// FailMsgID 消息发送失败事件特有。发送失败的消息msgid
	FailMsgID string `json:"fail_msgid,omitempty,omitempty"`
	...
	FailType int `json:"fail_type,omitempty,omitempty"`
	...
	StopType int `json:"stop_type,omitempty,omitempty"`
	// ChangeType 会话状态变更事件特有。变更类型，均为接待人员在企业微信客户端操作触发。1-从接待池接入会话 2-转接会话 3-结束会话 4-重新接入已结束/已转接会话
	ChangeType KfServiceState `json:"change_type,omitempty,omitempty"`
	// OldServicerUserID 会话状态变更事件特有。老的接待人员userid。仅change_type为2、3和4有值
	OldServicerUserID string `json:"old_servicer_userid,omitempty,omitempty"`
	// NewServicerUserid 会话状态变更事件特有。新的接待人员userid。仅change_type为1、2和4有值
	NewServicerUserid string `json:"new_servicer_userid,omitempty,omitempty"`
	// MsgCode 会话状态变更事件特有。用于发送事件响应消息的code，仅change_type为1和3时，会返回该字段。可用该msg_code调用发送事件响应消息接口给客户发送回复语或结束语。
	MsgCode string `json:"msg_code,omitempty,omitempty"`
	// RecallMsgID 撤回消息事件特有。 撤回的消息msgid
	RecallMsgID string `json:"recall_msgid,omitempty,omitempty"`
	// RejectSwitch 拒收客户消息变更事件特有。 拒收客户消息，1表示接待人员拒收了客户消息，0表示接待人员取消拒收客户消息
	RejectSwitch int `json:"reject_switch,omitempty,omitempty"`
}
...
type KfWechatChannels struct {
	...
	ShopNickName string `json:"shop_nickname,omitempty,omitempty"`
